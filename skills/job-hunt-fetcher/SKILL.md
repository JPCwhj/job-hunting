...
若实际结果少于 10 条，提取所有存在的卡片；若搜索结果为 0，跳过该轮次，在 `<work_dir>/output/<run_id>/search.summary.md` 的「失败列表」中记录 warning：`"[keyword]-[city] 搜索结果为空"`。

列表页读取完毕后，等待随机 **5-8 秒**（模拟用户在列表页浏览的停留），再进入 3.3 过滤流程。
