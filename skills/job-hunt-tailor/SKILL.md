...
- **技能排序**：识别简历中的技能概览板块（语义识别，不依赖板块名称，可能叫「专业技能」「核心能力」「技术栈」「能力概览」等），对照 JD，列出应前移的技能条目；总条数必须保持不变，只调整顺序。
...
3. 技能概览板块（语义识别，不依赖板块名称）：按 1.0 分析结论，把 JD 强调的技能条目移到最前，其余条目依次跟随，**总条数不得增减，只调整顺序**
