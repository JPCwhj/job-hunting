...
- **技能排序**：识别简历中的技能概览板块（语义识别，不依赖板块名称，可能叫「专业技能」「核心能力」「技术栈」「能力概览」等；若无标题，通过内容特征识别：连续排列的技能词/工具名/框架名），对照 JD，列出应前移的技能条目；**只输出「哪几条前移」的结论，不得对技能条目做任何改写、合并或拆分。原文在 Step 1.1 中原样复用。**
...
3. 技能概览板块（语义识别，不依赖板块名称；若简历无板块标题，通过内容特征识别：连续排列的技能词、工具名、框架名，通常以逗号/斜杠/换行分隔）：按 1.0 分析结论，把 JD 强调的技能条目移到最前，其余条目依次跟随。**严格约束：每个条目的原文必须一字不改地完整保留，不得改写、不得合并、不得拆分、不得删除任何条目；只允许调整条目在列表中的前后位置。**
