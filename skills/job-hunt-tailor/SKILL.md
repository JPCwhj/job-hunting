...
输出格式：完整 Markdown 简历，与主简历结构一致，只改内容不改结构。「结构」指：一级/二级标题名称及层次、各段落的存在与否（不增删段落）、**顶层章节的排列顺序（以原简历实际出现顺序为准，冻结，不得互换；如专业技能、工作经历、项目经历、教育背景等仅为举例说明章节类型，不代表其应有顺序）**、**每段经历内的条目数（不得删减任何子条目）**。章节内条目排序调整属于内容改动，需在 changelog.md 的「顺序调整」节中注明。
