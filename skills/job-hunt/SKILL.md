...
`work_dir` 按以下顺序确定：
1. 当前目录（`./`）下存在 `preferences.yaml` → 当前目录为 `work_dir`
2. 否则 → `~/.job-hunt/` 为 `work_dir`

`~/.job-hunt/` 若不存在，用 Bash 自动创建（`mkdir -p ~/.job-hunt`），**不停止、不提示用户**。
...
**向导第 6 步 — 排除关键词**
问：「有没有需要排除的 JD 关键词？（标题或正文含这些词就跳过，默认已排除"外包"和"驻场"，可追加或直接回复"默认就好"）」
等待用户回复后继续。
