...
根据用户提供方式处理：
- **发文件（.docx / .doc）**：调用 Skill 工具加载 `docx` skill 解析，转为 Markdown 文本
- **发文件（.md）**：直接读取文件内容
- **发文件（其他格式）**：告知用户「暂时只支持 .md、.docx、.doc 格式的简历文件。如果你的简历是 PDF，可以用 PDF 阅读器全选复制，然后直接把文字粘贴发给我。」停止执行，等待用户重新操作
- **文件路径（.docx / .doc）**：用 Bash 确认路径存在后，调用 `docx` skill 解析
- **文件路径（.md）**：用 Bash 读取文件内容
- **文件路径（其他格式）**：告知用户「暂时只支持 .md、.docx、.doc 格式的简历文件。如果你的简历是 PDF，可以用 PDF 阅读器全选复制，然后直接把文字粘贴发给我。」停止执行，等待用户重新操作
- **粘贴文本**：直接使用该文本内容
