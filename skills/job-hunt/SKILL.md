...
读取 `<work_dir>/.work/resume.md`，识别每段工作经历和项目经验下的**具体描述内容**（项目描述、职责条目、成果说明等 bullet points），对这些描述性内容逐段评估，输出结果。

**注意**：工作经历/项目的头部信息（公司名称、职位名称、在职时间段等）**不做评估**，直接跳过——这些是履历事实，不是需要优化的描述内容。
